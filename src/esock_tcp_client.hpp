 ...
 * async_tcp_client_hanndler_subclass
 * usage:
 * <code>
 ...
 *
 */
template <class subclass_t> struct async_tcp_connect_hanndler_t
{
    ...
    static inline void on_conn_complete_helper (net_engine_t *eng, int sock, void *arg)
    {
    ...
    static inline void on_conn_failed_helper (net_engine_t *eng, int sock, const int err, void *arg)
    {
        if (err == EINPROGRESS)
...
template <class subclass_t> struct async_tcp_client_hanndler_t
{
    /*  请实现下面这些函数
     void on_conn_connecting(net_engine_t *eng, int sock)
   ...
   */
    ...
    static inline void on_conn_complete_helper (net_engine_t *eng, int sock, void *arg)
    {
        ...
        if (sockpool.get_info (sock)->is_closed ()) return;

        if (-1 == eng->epoll_add_sock (sock, EPOLLIN, (void *)on_conn_recvable_helper, (void *)on_conn_sendable_helper, arg))
        {
